...
function Scanner() {
  ...
  const params = useMemo(() => ({ sort, search: search || undefined, limit: 20 }), [search, sort]);
  const tokens = useListTokens(params, { query: { queryKey: getListTokensQueryKey(params), refetchInterval: 30_000, refetchOnWindowFocus: true } });
  const client = useQueryClient();
